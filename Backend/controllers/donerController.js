...
const registerDoner = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Registration successful', doner: newDoner });
  } catch (error) {
...
const donerLogin = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Donor login successful', userType: 'doner' });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};

module.exports = { donerLogin };
module.exports = { registerDoner };
